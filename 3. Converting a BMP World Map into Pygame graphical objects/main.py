...
def createControlableObject(PLAYER_SIZE):
    # Create a rectangle object
    mover = Mover(10, 20, PLAYER_SIZE, PLAYER_SIZE)
    return mover
...
def drawControlableObject(player):
    pygame.draw.rect(screen, (255,112,133), player)
...
#load map in .bmp:
mapBMP = pygame.image.load("map.bmp")
if mapBMP:
    print("map loaded")
else:
    sys.exit(1)

MATRIX_HEIGHT = mapBMP.get_height()
MATRIX_WIDTH = mapBMP.get_width()
FIELD_SIZE = 20
PLAYER_SIZE = 19
#create human controlled object with keyboard:
player = createControlableObject(PLAYER_SIZE)

#turn the 2D map into 1D array (at the beginning i was using 1D):
map1D = []
print("MATRIX_HEIGHT: ", MATRIX_HEIGHT)
print("MATRIX_WIDTH: ", MATRIX_WIDTH)
for y in range(MATRIX_HEIGHT):
    for x in range(MATRIX_WIDTH):
        map1D.append(mapBMP.get_at((x,y)))


world = World(screen, map1D, MATRIX_HEIGHT, MATRIX_WIDTH,FIELD_SIZE)
...
startNode = worldNodesList[4 + 4 * MATRIX_WIDTH]
